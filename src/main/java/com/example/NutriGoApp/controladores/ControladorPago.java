...
public class ControladorPago {
    ...
    //Buscar por id
    @GetMapping("/{id}")
    public ResponseEntity<?> buscarIdPago(@RequestBody Integer id_pago){
        try{
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(this.pagoServicio.buscarPago(id_pago));
        } catch (Exception error) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(error.getMessage());
    ...
    //Modificar
    @PutMapping("/{id}")
    public ResponseEntity<?> modificarPago(@PathVariable Integer id_pago, @RequestBody Pago datosPago){
        try{
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(this.pagoServicio.modificarPorId(id_pago,datosPago));
        } catch (Exception error) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(error.getMessage());
    ...
    //Eliminar
    @DeleteMapping("/{id}")
    public ResponseEntity<?> eliminarPago(@PathVariable Integer id_pago){
        try{
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(this.pagoServicio.eliminarPorId(id_pago));
        } catch (Exception error) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(error.getMessage());
