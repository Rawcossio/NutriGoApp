...
public class ControladorTienda {
    ...
    public ResponseEntity<?> buscarPorId(@PathVariable Integer id){
        try{
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(this.tiendaServicio.buscarPorId(id));
        } catch (Exception error) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body(error.getMessage());
    ...
    //Modificar
    @PutMapping("/{id}")
    public  ResponseEntity<?> modificarTienda(@PathVariable Integer id, @RequestBody Tienda datosTienda){
        try{
            return ResponseEntity
                    .status(HttpStatus.OK)
    ...
    public ResponseEntity<?>eliminarTienda(@PathVariable Integer id){
        try{
            return ResponseEntity.status(HttpStatus.OK).body(this.tiendaServicio.eliminarTienda(id));
        } catch (Exception error) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error.getMessage());
        }
    }
